...
function Index() {
  ...
  const ImageData = useQuery(
    ["packarma_work_with_images"],
    () => {
      return getAllData("packarma_work_with_images");
    },
    {
      staleTime: 10000 * 60,
    }
  ...
  const handleSubmitImage = (e) => {
    ...
    const resp = uploadImage(image, `packarma_WorkWith`);
    resp.then((res) => {
      if (res.message === "success") {
        const data = {
          caption: caption,
        ...
        uploadData(data, "packarma_work_with_images").then((res) => {
          if (res.message === "success") {
            alert("Image Uploaded");
            queryClient.invalidateQueries("packarma_work_with_images");
          }
        });
      } else {
              ...
              <div key={index} style={{ width: "200px", margin: "10px" }}>
                ...
                >
                  ...
                  <AiFillDelete
                    ...
                    onClick={() => {
                      deleteDataById(`packarma_work_with_images/${item.id}`).then(
                        (res) => {
                          if (res.message === "success") {
                            alert("Deleted");
                            queryClient.invalidateQueries("packarma_work_with_images");
